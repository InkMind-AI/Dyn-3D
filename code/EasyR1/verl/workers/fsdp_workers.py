...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if self.model_config.model_type == "internvl_chat":
        ...
        if is_lora_model:
            ...
            lora_kwargs = {
                "task_type": None if self.model_config.model_type == "internvl_chat" else TaskType.CAUSAL_LM,
                "r": model_config.lora.rank,
                "lora_alpha": model_config.lora.alpha,
                "target_modules": target_modules,
            }
            if "exclude_modules" in peft.LoraConfig.__init__.__code__.co_varnames:
                lora_kwargs["exclude_modules"] = model_config.lora.exclude_modules
            elif model_config.lora.exclude_modules:
                self.print_rank0(
                    "Installed PEFT does not support lora.exclude_modules; "
                    "continuing without that argument."
                )
        ...
        if model_config.freeze_vision_tower:
            if hasattr(model, "model") and hasattr(model.model, "visual"):  # transformers >= 4.52.0
                ...
                fsdp_config.use_orig_params = True
                self.print_rank0("Vision tower is set to not trainable.")
            elif hasattr(model, "vision_model"):  # InternVL
                model.vision_model.requires_grad_(False)
                if hasattr(model, "mlp1"):
                    model.mlp1.requires_grad_(False)
                fsdp_config.use_orig_params = True
                self.print_rank0("InternVL vision tower and projector are set to not trainable.")
            else:
