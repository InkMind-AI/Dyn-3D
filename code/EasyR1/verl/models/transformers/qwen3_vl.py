...
def qwen3_vl_base_forward(
    ...
    **kwargs,
    ...
    if isinstance(position_ids, torch.Tensor) and position_ids.ndim == 1:
        position_ids = position_ids.view(1, -1).unsqueeze(0).expand(4, 1, -1)
        kwargs["position_ids"] = position_ids
    elif isinstance(position_ids, torch.Tensor) and position_ids.ndim == 2:
        position_ids = position_ids.unsqueeze(0).expand(4, -1, -1)
        kwargs["position_ids"] = position_ids
    elif isinstance(position_ids, torch.Tensor) and (position_ids.ndim != 3 or position_ids.size(0) != 4):
