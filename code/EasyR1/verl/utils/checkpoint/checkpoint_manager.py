...
def remove_obsolete_ckpt(
    ...
    pinned_steps = set()
    pinned_steps_env = os.environ.get("EASYR1_PINNED_CHECKPOINT_STEPS", "")
    for raw_step in re.split(r"[,\s]+", pinned_steps_env.strip()):
        if not raw_step:
            continue
        try:
            pinned_steps.add(int(raw_step))
        except ValueError:
            print(f"Ignoring invalid pinned checkpoint step: {raw_step}")
    ...
    for folder in os.listdir(path):
        if match := re.match(pattern, folder):
            ...
            if step < global_step and step not in pinned_steps:
